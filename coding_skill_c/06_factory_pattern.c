 ...
 * ============================================================================
 * 
 * 1. 배열 인덱스 대신 ID/이름으로 접근
 *    - sensors[0] ❌  → SensorRegistry_GetById(101) ✅
 *    - sensors[2] ❌  → SensorRegistry_GetByName("temp1") ✅
 * 
 * 2. 객체 생명주기 중앙 관리
 *    - 생성/등록/검색/해제를 한 곳에서 관리
 * 
 * 3. 확장성
 *    - 센서 개수가 동적으로 변해도 배열 크기 신경 안 써도 됨
 * 
 * 4. 유지보수성
 *    - 센서 추가/삭제 시 인덱스 재조정 불필요
 * 
 * ============================================================================
...
/* ============================================================================
 * 💡 추가 학습: Registry 패턴 - ID/이름으로 객체 관리하기
 * ============================================================================
 * 
 * Factory로 생성한 객체를 배열 인덱스 대신
 * ID나 이름으로 관리하고 싶다면?
 * 
 * 문제점:
 *   Sensor* sensors[10];
 *   sensors[0] = SensorFactory_Create(..., 101);
 *   sensors[1] = SensorFactory_Create(..., 102);
 *   
 *   // 나중에 ID 101 센서를 사용하려면?
 *   sensors[0]->read(...);  // 인덱스를 기억해야 함!
 * 
 * 해결책: Registry 패턴
 *   - 객체를 생성 후 Registry에 등록
 *   - ID나 이름으로 검색하여 사용
 * 
 * ============================================================================
 * Registry 패턴 구현 예시 (참고용)
 * ============================================================================
 * 
 ...
 * } SensorRegistry;
 * 
 * static SensorRegistry g_registry = {0};
 * 
 * // 센서 등록
 * int Registry_Register(Sensor* sensor) {
 *     g_registry.sensors[g_registry.count++] = sensor;
 *     return 0;
 * }
 * 
 * // ID로 검색
 * Sensor* Registry_GetById(int id) {
 *     for (int i = 0; i < g_registry.count; i++) {
 *         if (g_registry.sensors[i]->sensor_id == id) {
 *             return g_registry.sensors[i];
 *         }
 *     }
 *     return NULL;
 * }
 * 
 * // 이름으로 검색
 * Sensor* Registry_GetByName(const char* name) {
 *     for (int i = 0; i < g_registry.count; i++) {
 *         if (strcmp(g_registry.sensors[i]->name, name) == 0) {
 *             return g_registry.sensors[i];
 *         }
 *     }
 *     return NULL;
 ...
 * Registry_Register(temp);
 * 
 * // 2. ID로 접근
 * Sensor* s = Registry_GetById(101);
 * if (s) {
 *     float value = s->read(s);
 * }
 * 
 * // 3. 이름으로 접근
 * Sensor* s2 = Registry_GetByName("Temperature_101");
 * if (s2) {
 *     float value = s2->read(s2);
 * }
 * 
 * ============================================================================
 * Registry 패턴의 장단점
 ...
 * 
 * ✅ 장점:
 *   1. 배열 인덱스 불필요 - 직관적인 ID/이름 접근
 *   2. 유지보수 용이 - 센서 추가/삭제 시 인덱스 재조정 불필요
 *   3. 확장성 - 동적 개수 변화 대응
 * 
 * ⚠️ 단점:
 *   1. 검색 오버헤드 - 배열 순회 필요 (O(n))
 *   2. 메모리 오버헤드 - Registry 구조체 추가
 *   3. 복잡도 증가 - 간단한 경우 불필요
 * 
 * ============================================================================
 * 언제 사용하나?
